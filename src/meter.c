...
static void reader_disconnect_handler(int)
{
    if (fifo_fd != -1)
    {
        close(fifo_fd);
...
static int init(const char *name, int max, int show, int size, int log_f, int log_y, int smooth_f, int window)
{
    (void)size;
    (void)log_f;
    (void)log_y;
